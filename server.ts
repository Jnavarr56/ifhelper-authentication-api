...
const API_BASE_URL: string = typeof process.env.API_BASE_URL === 'string' ? process.env.API_BASE_URL : '/api';
const PATHNAME: string = '/authentication';
...
// public
app.post(`${API_BASE_URL}${PATHNAME}/sign-in`, async (req: RequestWithIpInfo, res) => {
...
// token required
app.get(`${API_BASE_URL}${PATHNAME}/authorize`, async (req: RequestWithIpInfo, res) => {
	...
	const isBlacklisted: boolean = (await TokenBlacklistCache.getKey(accessToken)) !== null;
	...
	jwt.verify(accessToken, JWT_SECRET_KEY, async (error, decodedAccessToken: any) => {
		...
		if (!req.cookies[REFRESH_TOKEN_COOKIE_NAME]) {
			const tokenStore: any = await TokenStore.findOne({ access_token: accessToken });
...
// token required
app.get(`${API_BASE_URL}${PATHNAME}/refresh`, async (req: RequestWithIpInfo, res) => {
	const { token: accessToken } = req;
...
app.post(`${API_BASE_URL}${PATHNAME}/sign-out-all-devices`, async (req, res) => {
	const { token: accessToken } = req;
	...
	const cachedPayload: any = await AuthTokenCache.getKey(accessToken);
	if (cachedPayload !== null) {
		const { access_type, authenticated_user } : 
		{ access_type: string, authenticated_user: { _id: string } } = cachedPayload;
		...
		for (let store of storeArr) {
			const notYetBlacklisted: boolean = await TokenBlacklistCache
				.getKey(store.access_token).then(payload => payload === null);

			if (notYetBlacklisted) {
				const ttl: number = Math.ceil((store.access_token_exp_date - store.createdAt) / 1000);
				await TokenBlacklistCache.setKey(accessToken, {}, ttl);
			}
	...
	jwt.verify(accessToken, JWT_SECRET_KEY, async (error, decodedToken: any) => {
		...
		const { access_type,  authenticated_user, iat, exp } : 
		{ 
			access_type: string, 
...
const dbOptions: ConnectionOptions = { useNewUrlParser: true, useUnifiedTopology: true };
